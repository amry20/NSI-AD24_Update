...
ZeroError = False
print("Module waveserver.py ini digunakan untuk mengatasi masalah data NSI tidak bisa di download. Module ini juga masih dapat digunakan untuk data yang tidak error")
print("")
RETURNFLAG_KEY = {
    'F': 'success',
...
class TraceBuf2(object):
    ...
    def parse_header(self, head):
        ...
        dtype = head[-7:-5]
        if not ((dtype == b'i4') or (dtype == b'i2')):
            return 0
...
def read_wave_server_v(server, port, scnl, start, end, timeout=None,
    ...
    while bytesread and p < dat_len:
        ...
        new_tb = TraceBuf2()
        ret = new_tb.parse_header(dat[p:p + 64])
        if ret > 0:
            p += 64
